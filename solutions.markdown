---
layout: page
title: "Solutions"
permalink: /solutions
nav_display: true
sidebar_headers: false
---


<div class="card-group">

{% include card-begin.html
card_type="solutions-card" 
header_txt="1 o+n 1 Sessions" 
title_txt="Why?" 
%}
<p class="card-text">In this course, you'll learn....</p>
<p class="card-text">Do you need a ...</p>
<p class="card-text">something thingthing</p>
  <p class="card-text"><strong>Who is this for?</strong>
  <ul>
    <li>Small businesses or start-ups with new or improved products or services.  </li>
  </ul>
</p>
<p class="card-text"><strong>Time Committment</strong>
  <ul>
    <li>Seminar or Mentoring: 60min.</li>
    <li>Workshop:  90min</li>
  </ul>
</p>
<p class="card-text"><strong>Price</strong>
  <ul>
    <li>Seminar or Mentoring: $100</li>
    <li>Workshop for up to 6 people: $950</li>
  </ul>
</p>
<p class="card-text"><strong>Format</strong>
  <ul>
    <li>The skills and materials are learned face to face, either in real life or via videoconference with the individual or group taking the course.</li>
    <li>The personalization you choose may include skills practices between modules, and/or mentoring session(s).</li> 
  </ul>
</p>
{% include card-end.html
url="/service_foo"
button="Let's Chat!"
%}

</div>
<div class="card-group">

{% include card-begin.html
card_type="solutions-card" 
header_txt="Who Needs a Skills Refresh?" 
title_txt="Ongoing training: Solidifying your foundation and/or building on it."
%}
<p class="card-text">Ongoing training sessions are important when you or your employees are learning about one or more topics either from scratch or with little practical knowledge.</p>
<p class="card-text">Our ongoing trainings can focus on one specific topic, breaking it down into easily digestible chunks, or they can build on each other, as is the case with our UX Research for Nonpractitioners series.</p> 
<p class="card-text">How do you know whether a series or stand-alone session is right for you?</p>
<p class="card-text">In this course, you'll learn....</p>
<p class="card-text">Do you need a ...</p>
<p class="card-text">something thingthing</p>
  <p class="card-text"><strong>Who is this for?</strong>
  <ul>
    <li>Small businesses or start-ups with new or improved products or services.  </li>
  </ul>
</p>
<p class="card-text"><strong>Time Committment</strong>
  <ul>
    <li>Seminar or Mentoring: 60min.</li>
    <li>Workshop:  90min</li>
  </ul>
</p>
<p class="card-text"><strong>Price</strong>
  <ul>
    <li>Seminar or Mentoring: $100</li>
    <li>Workshop for up to 6 people: $950</li>
  </ul>
</p>
<p class="card-text"><strong>Format</strong>
  <ul>
    <li>The skills and materials are learned face to face, either in real life or via videoconference with the individual or group taking the course.</li>
    <li>The personalization you choose may include skills practices between modules, and/or mentoring session(s).</li> 
  </ul>
</p>
{% include card-end.html
url="/service_foo"
button="Refresh Me!"
%}


{% include card-begin.html
card_type="solutions-card" 
header_txt="Privacy & the Ethical Use of Data" 
title_txt="Available as a seminal, workshop, training series, and mentoring session"
%}
<p class="card-text">In this course, you'll learn....</p>
<p class="card-text">Do you need a ...</p>
<p class="card-text">something thingthing</p>
  <p class="card-text"><strong>Who is this for?</strong>
  <ul>
    <li>Small businesses or start-ups with new or improved products or services.  </li>
  </ul>
</p>
<p class="card-text"><strong>Time Committment</strong>
  <ul>
    <li>Seminar or Mentoring: 60min.</li>
    <li>Workshop:  90min</li>
  </ul>
</p>
<p class="card-text"><strong>Price</strong>
  <ul>
    <li>Seminar or Mentoring: $100</li>
    <li>Workshop for up to 6 people: $950</li>
  </ul>
</p>
<p class="card-text"><strong>Format</strong>
  <ul>
    <li>The skills and materials are learned face to face, either in real life or via videoconference with the individual or group taking the course.</li>
    <li>The personalization you choose may include skills practices between modules, and/or mentoring session(s).</li> 
  </ul>
</p><p class="card-text">sentence two</p>
<p class="card-text">sentence three
  <ul>
    <li>List one</li>
    <li>List item two</li>
  </ul>
</p>
{% include card-end.html
url="/service_foo"
button="Let's Do This!"
%}

{% include card-begin.html
card_type="solutions-card" 
header_txt="Help Team Members Embrace New Tech Without Losing Thier Minds " 
title_txt="Available as a seminar, workshop, or mentoring session."
%}
<p class="card-text">sentence one</p>
<p class="card-text">sentence two</p>
<p class="card-text">sentence three
  <ul>
    <li>List one</li>
    <li>List item two</li>
  </ul>
</p>
{% include card-end.html
url="/service_foo"
button="Ready to Help!"
%}


</div>
<div class="card-group">


{% include card-begin.html
card_type="solutions-card" 
header_txt="Remote Work and Cybersecurity" 
title_txt="Available as a seminar or mentoring session."
%}
<p class="card-text">In this course, you'll learn....</p>
<p class="card-text">Do you need a ...</p>
<p class="card-text">something thingthing</p>
  <p class="card-text"><strong>Who is this for?</strong>
  <ul>
    <li>Small businesses or start-ups with new or improved products or services.  </li>
  </ul>
</p>
<p class="card-text"><strong>Time Committment</strong>
  <ul>
    <li>Seminar or Mentoring: 60min.</li>
    <li>Workshop:  90min</li>
  </ul>
</p>
<p class="card-text"><strong>Price</strong>
  <ul>
    <li>Seminar or Mentoring: $100</li>
    <li>Workshop for up to 6 people: $950</li>
  </ul>
</p>
<p class="card-text"><strong>Format</strong>
  <ul>
    <li>The skills and materials are learned face to face, either in real life or via videoconference with the individual or group taking the course.</li>
    <li>The personalization you choose may include skills practices between modules, and/or mentoring session(s).</li> 
  </ul>
</p>
{% include card-end.html
url="/service_foo"
button="Let's Go!"
%}
 

{% include card-begin.html
card_type="solutions-card" 
header_txt="Understanding Ethics in UX or Academic Research" 
title_txt="Available as a seminar, workshop, or mentoring session."
%}
<p class="card-text">In this course, you'll learn....</p>
<p class="card-text">Do you need a ...</p>
<p class="card-text">something thingthing</p>
  <p class="card-text"><strong>Who is this for?</strong>
  <ul>
    <li>Small businesses or start-ups with new or improved products or services.  </li>
  </ul>
</p>
<p class="card-text"><strong>Time Committment</strong>
  <ul>
    <li>Seminar or Mentoring: 60min.</li>
    <li>Workshop:  90min</li>
  </ul>
</p>
<p class="card-text"><strong>Price</strong>
  <ul>
    <li>Seminar or Mentoring: $100</li>
    <li>Workshop for up to 6 people: $950</li>
  </ul>
</p>
<p class="card-text"><strong>Format</strong>
  <ul>
    <li>The skills and materials are learned face to face, either in real life or via videoconference with the individual or group taking the course.</li>
    <li>The personalization you choose may include skills practices between modules, and/or mentoring session(s).</li> 
  </ul>
</p>
{% include card-end.html
url="/service_foo"
button="Let's Go!"
%}


{% include card-begin.html
card_type="solutions-card" 
header_txt="Specific Projects" 
title_txt="Types of Projects Include, but aren't limited to:"
%}
<p class="card-text">In this course, you'll learn....</p>
<p class="card-text">Do you need a ...</p>
<p class="card-text">something thingthing</p>
  <p class="card-text"><strong>Who is this for?</strong>
  <ul>
    <li>Small businesses or start-ups with new or improved products or services.  </li>
  </ul>
</p>
<p class="card-text"><strong>Time Committment</strong>
  <ul>
    <li>Seminar or Mentoring: 60min.</li>
    <li>Workshop:  90min</li>
  </ul>
</p>
<p class="card-text"><strong>Price</strong>
  <ul>
    <li>Seminar or Mentoring: $100</li>
    <li>Workshop for up to 6 people: $950</li>
  </ul>
</p>
<p class="card-text"><strong>Format</strong>
  <ul>
    <li>The skills and materials are learned face to face, either in real life or via videoconference with the individual or group taking the course.</li>
    <li>The personalization you choose may include skills practices between modules, and/or mentoring session(s).</li> 
  </ul>
</p>
{% include card-end.html 
url="/service_foo"
button="Excited to Start!"
%}

</div>
<div class="card-group">

{% include card-begin.html
card_type="solutions-card" 
header_txt="Identifying Gaps that Expose Cybersecurity Vulnerabilities" 
title_txt="BAR"
%}
<p class="card-text">In this course, you'll learn....</p>
<p class="card-text">Do you need a ...</p>
<p class="card-text">something thingthing</p>
  <p class="card-text"><strong>Who is this for?</strong>
  <ul>
    <li>Small businesses or start-ups with new or improved products or services.  </li>
  </ul>
</p>
<p class="card-text"><strong>Time Committment</strong>
  <ul>
    <li>Seminar or Mentoring: 60min.</li>
    <li>Workshop:  90min</li>
  </ul>
</p>
<p class="card-text"><strong>Price</strong>
  <ul>
    <li>Seminar or Mentoring: $100</li>
    <li>Workshop for up to 6 people: $950</li>
  </ul>
</p>
<p class="card-text"><strong>Format</strong>
  <ul>
    <li>The skills and materials are learned face to face, either in real life or via videoconference with the individual or group taking the course.</li>
    <li>The personalization you choose may include skills practices between modules, and/or mentoring session(s).</li> 
  </ul>
</p>
{% include card-end.html
url="/service_foo"
button="Defend the Realm!"
%}


{% include card-begin.html
card_type="solutions-card" 
header_txt="Specific Skillbuilding Topics" 
title_txt="Available as a seminar, workshop, training series, or mentoring session."
%}
<p class="card-text">In this course, you'll learn....</p>
<p class="card-text">Do you need a ...</p>
<p class="card-text">something thingthing</p>
  <p class="card-text"><strong>Who is this for?</strong>
  <ul>
    <li>Small businesses or start-ups with new or improved products or services.  </li>
  </ul>
</p>
<p class="card-text"><strong>Time Committment</strong>
  <ul>
    <li>Seminar or Mentoring: 60min.</li>
    <li>Workshop:  90min</li>
  </ul>
</p>
<p class="card-text"><strong>Price</strong>
  <ul>
    <li>Seminar or Mentoring: $100</li>
    <li>Workshop for up to 6 people: $950</li>
  </ul>
</p>
<p class="card-text"><strong>Format</strong>
  <ul>
    <li>The skills and materials are learned face to face, either in real life or via videoconference with the individual or group taking the course.</li>
    <li>The personalization you choose may include skills practices between modules, and/or mentoring session(s).</li> 
  </ul>
</p>
{% include card-end.html 
url="/service_foo"
button="Ready to Learn!"
%}


{% include card-begin.html
card_type="solutions-card" 
header_txt="All You Ever Wanted to Know about Focus Groups" 
title_txt="BAR"
%}
<p class="card-text">sentence one</p>
<p class="card-text">sentence two</p>
<p class="card-text">sentence three
  <ul>
    <li>List one</li>
    <li>List item two</li>
  </ul>
</p>
{% include card-end.html 
url="/service_foo"
button="I'm New to the Throne"
%}

</div>
<div class="card-group">

{% include card-begin.html
card_type="solutions-card" 
header_txt="BAR" 
title_txt="After recruitment: Creating and sustaining relationships with clients/usrs."
%}
<p class="card-text">In this course, you'll learn....</p>
<p class="card-text">Do you need a ...</p>
<p class="card-text">something thing thing</p>
<p class="card-text"><strong>Who is this for?</strong>
<ul>
<li>Non-researchers who work with users/participants</li>
<li>Researchers of all skill levels.</li>
</ul>
</p>
<p class="card-text"><strong>Time Committment</strong>
<ul>
<li>Seminar or Mentoring: 60min.</li>
<li>Workshop:  90min</li>
</ul>
</p>
<p class="card-text"><strong>Price</strong>
<ul>
<li>Seminar or Mentoring: $100</li>
<li>Workshop for up to 6 people: $950</li>
</ul>
</p>
<p class="card-text"><strong>Format</strong>
<ul>
<li>The skills and materials are learned face to face, either in real life or via videoconference with the individual or group taking the course.</li>
<li>The personalization you choose may include skills practices between modules, and/or mentoring session(s).</li> 
</ul>
</p>
{% include card-end.html
url="/service_foo"
button="Let's Chat!"
%}


{% include card-begin.html
card_type="solutions-card" 
header_txt="Everyone's Democratizing Research but Idk Where to Start" 
title_txt="Available as a seminar, workshop, retainer-based consulting, or a stand-alone project."
%}
<p class="card-text">Research democratization is becoming more and more popular as organizations look to cut costs and cross-train their designers and/or other non-research personnel. In the seminar, workshop, or mentoring session, you'll learn the basics of what is needed to implement a research democratization initiative for non-research UX designers, project managers, etc. This is a great option if you're already familiar with industry standards and best practices, know which methodologies need to be taught and how to do it, and have tasked personnel who are confident in their ability to implementing the initiative across your organization.</p>
<p class="card-text">Have you been tasked with creating and implementing a UX research democratization program but are unsure of where to start or how to continue it? Would you like to Critical Path to be just a phone call or e-mail away? By choosing a consulting retainer, you and your team will have front-of-the-line access to our experts who will answer any and all questions or issues that may arise.</p>
<p class="card-text">Most smaller businesses and start-ups do not have personnel with the bandwidth to implement a project with so many moving parts. If this sounds like your experience, Critical Path specializes in creating and implementing a democratization process specifically for your team. While acting as your Research Ops manager, we'll ensure your team understands industry standards, ethics, how to decide on best-fit methods to allow you to run as lean as possible without sacrificing actionable insights. We'll also introduce guidelines for best practices, documentation, and relaying findings to stakeholders, and so much more!</p>
<p class="card-text"><strong>Who is this for?</strong>
<ul>
<li>Anyone interested in a cross-training research initiative for UX non-researchers.</li>
</ul>
</p>
<p class="card-text"><strong>Time Committment</strong>
<ul>
<li>Seminar: 2hrs.</li>
<li>Workshop:  3hrs.</li>
<li>Retainers: Consistent access to CP experts for guidance in monthly, quarterly, or other increments.</li>
<li>Project: Critical Path creates and implements the democratization initiative with you and your team.</li> 
</ul>
</p>
<p class="card-text"><strong>Price</strong>
<ul>
<li>Seminar for up to 6 people: $1,200</li>
<li>Workshop for up to 6 people: $2,500</li>
<li>Retainer: starts at $3,000 per month, depending on amount of access wanted</li>
<li>Critical Path Democratization Program Creation and Implementation: starts at $15,000, depending on complexity/breadth of program wanted</li> 
</ul>
</p>
<p class="card-text"><strong>Format</strong>
<ul>
<li>All solutions above are available both in-person and live videoconferencing.</li>
<li>The personalization you choose may include skills practices between modules, and/or one-on-one mentoring session(s) to aid with internalization of information.</li> 
</ul>
</p>
{% include card-end.html 
url="/service_foo"
button="Ready to Go!"
%}
 
{% include card-begin.html
card_type="solutions-card" 
header_txt="How Kano Surveys Ensure User Delight" 
title_txt="Available as a seminar, workshop, or mentoring session"
%}
<p class="card-text">In this course, you'll learn....</p>
<p class="card-text">Do you need a ...</p>
<p class="card-text">something thingthing</p>
  <p class="card-text"><strong>Who is this for?</strong>
  <ul>
    <li>Small businesses or start-ups with new or improved products or services.  </li>
  </ul>
</p>
<p class="card-text"><strong>Time Committment</strong>
  <ul>
    <li>Seminar or Mentoring: 60min.</li>
    <li>Workshop:  90min</li>
  </ul>
</p>
<p class="card-text"><strong>Price</strong>
  <ul>
    <li>Seminar or Mentoring: $100</li>
    <li>Workshop for up to 6 people: $950</li>
  </ul>
</p>
<p class="card-text"><strong>Format</strong>
  <ul>
    <li>The skills and materials are learned face to face, either in real life or via videoconference with the individual or group taking the course.</li>
    <li>The personalization you choose may include skills practices between modules, and/or mentoring session(s).</li> 
  </ul>
</p>
{% include card-end.html
url="/service_foo"
button="I Need to Know!"
%}

</div>
<div class="card-group">

{% include card-begin.html
card_type="solutions-card" 
header_txt="Is WFH *Really* Better Than RTO?" 
title_txt="Available as a seminar, workshop, or mentoring session."
%}
<p class="card-text">In this course, you'll learn....</p>
<p class="card-text">Do you need a ...</p>
<p class="card-text">something thingthing</p>
  <p class="card-text"><strong>Who is this for?</strong>
  <ul>
    <li>Executives, managers</li>
    <li>Anyone thinking of adopting hybrid or work from home arrangements.</li> 
  </ul>
</p>
<p class="card-text"><strong>Time Committment</strong>
  <ul>
    <li>Seminar or Mentoring: 60min.</li>
    <li>Workshop:  90min</li>
  </ul>
</p>
<p class="card-text"><strong>Price</strong>
  <ul>
    <li>Seminar or Mentoring: $100</li>
    <li>Workshop for up to 6 people: $950</li>
  </ul>
</p>
<p class="card-text"><strong>Format</strong>
  <ul>
    <li>The skills and materials are learned face to face, either in real life or via videoconference with the individual or group taking the course.</li>
    <li>The personalization you choose may include skills practices between modules, and/or mentoring session(s).</li> 
  </ul>
</p>
{% include card-end.html 
url="/service_foo"
button="I'm So Ready!"
%}


{% include card-begin.html
card_type="solutions-card" 
header_txt="Diversity. Equity. Inclusion: Just the Basics"
title_txt="Available as a seminar or mentoring session."
%}
<p class="card-text">In this course, you'll learn....</p>
<p class="card-text">Do you need a ...</p>
<p class="card-text">something thingthing</p>
<p class="card-text"><strong>Who is this for?</strong>
<ul>
<li>Professionals in any industry, whether as an introduction or as a refresh</li> 
</ul>
</p>
<p class="card-text"><strong>Time Committment</strong>
<ul>
<li>2 hours, 60min in each part. This can be divided up up different days.</li> 
</ul>
</p>
<p class="card-text"><strong>Price</strong>
<ul>
<li>Mentoring: $100</li>
<li>Seminar for up to 5 people begins at $400, depending on level of personalization</li>
</ul>
</p>
<p class="card-text"><strong>Format</strong>
<ul>
<li>The skills and materials are learned face to face, either in real life or via videoconference with the individual or group taking the course.</li>
<li>The personalization you choose skills practices between modules, and/or mentoring session(s).</li> 
</ul>
</p>
{% include card-end.html 
url="/service_foo"
button="I'm So Ready!"
%}


{% include card-begin.html
card_type="solutions-card" 
header_txt="I Want It All!: The Full Course" 
title_txt="Available as a training series only, however skills level and timeline are adjustable."
%}
<p class="card-text">In this course, you'll learn one-on-one the role of the researcher, ethics and fundamental skills, how to plan research based on project type, how and why quantitative analysis should be part of your work, and the current role of AI in reputable research.</p>
<p class="card-text">Do you need a research project for your portfolio or research guidance on theses? We pride ourselves on personalizing our research series to meet your specific needs.</p>
<p class="card-text"><strong>Who is this for?</strong>
  <ul>
    <li>student or beggining researchers</li> 
    <li>UX professionals in non-research roles</li>
  </ul>
</p>
<p class="card-text"><strong>Time Committment</strong>
  <ul>
    <li>Committment time varies. Let's talk about your goals and time to dedicate to the course and adjust it based on your needs.</li> 
  </ul>
</p>
<p class="card-text"><strong>Price</strong>
  <ul>
    <li>Committment time varies. Let's talk about your goals and time to dedicate to the course and adjust it based on your needs.</li> 
  </ul>
</p>
<p class="card-text"><strong>Format</strong>
  <ul>
    <li>The skills and materials are learned face to face, either in real life or via videoconference with the individual or group taking the course.</li>
    <li>The personalization you choose may include workshopping, skills practices between modules, and/or mentoring session(s).</li> 
  </ul>
</p>
{% include card-end.html 
url="/service_foo"
button="I'm So Ready!"
%}

</div><!-- /.card-group -->